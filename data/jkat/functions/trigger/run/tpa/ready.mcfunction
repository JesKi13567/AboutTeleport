...
# 查询有无接受
execute as @a[distance=.1..,scores={jkat.tpa.flag.ok=1}] if score @s jkat.tpa.id.r = @p[tag=jkat_tpa_self] jkat.id run tag @s add jkat_tpa_target
...
scoreboard players set @a[tag=jkat_tpa_target] jkat.tpa.id.r 0
